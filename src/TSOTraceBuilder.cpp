...
void TSOTraceBuilder::end_of_thread(int proc, int aux){
  Thread &currth=threads[ipid(proc,aux)];
  currth.available = false;
  if(0 <= currth.handler_id){
    if(!threads[currth.handler_id].posts.empty()){
      IPid next_msg = threads[currth.handler_id].posts.front();
      threads[currth.handler_id].posts.pop();
      threads[next_msg].available = true;
    }else threads[currth.handler_id].ready_to_receive = true;
  }
}
...
void TSOTraceBuilder::post(const int tgt_th) {
  ...
  if(threads[tpid].ready_to_receive){
    threads[threads.size()-2].available = true;
  }else{
...
void TSOTraceBuilder::exec(){
  ...
  IPid ipid = curev().iid.get_pid();
  threads[ipid].available = false;
  if(threads[ipid].posts.empty()){
    threads[ipid].ready_to_receive = true;
    return;
  }
  IPid first_message = threads[ipid].posts.front();
  threads[first_message].available = true;
...
void TSOTraceBuilder::record_symbolic(SymEv event){
  if(dryrun) {
    ...
    if(event.kind == SymEv::POST) return;
...
bool TSOTraceBuilder::redundant_wakeup_seq(std::map<int,Event> wakeup_ev_seq,
  ...
  std::vector<std::pair<IID<IPid>,VClock<IPid>>> post_sleep;
  auto it = wakeup_ev_seq.begin();
  for(int i = 0; i <= ins_point; i++){
    for(auto s_it = post_sleep.begin(); s_it != post_sleep.end(); s_it++){
      if(s_it->first == it->second.iid) {
	return true;
      }
      if(s_it->second.gt(it->second.clock)){
  ...
  for(int i = ins_point+1; i < wakeup_ev_seq.size(); i++){
    for(auto s_it = post_sleep.begin(); s_it != post_sleep.end(); s_it++){
      if(s_it->first == it->second.iid) return true;
      if(s_it->second.gt(it->second.clock)){
        s_it = post_sleep.erase(s_it);
        s_it--;
      }
...
wakeup_sequence(const Race &race, std::map<int,Event> &wakeup_ev_seq) const{
  ...
  for (int k = i + 1; k < int(prefix.len()); ++k){
    if (!first.clock.leq(prefix[k].clock)) {
	  ...
	  notobs.back().cpid = threads[prefix[k].iid.get_pid()].cpid;
        }
      }
    }
  }
  ...
  v.back().cpid = threads[prefix[j].iid.get_pid()].cpid;
  wakeup_ev_seq.at(j).eom_before.clear();

  /* Include messages in the wakeup sequence */
  if (race.kind == Race::NONBLOCK &&
