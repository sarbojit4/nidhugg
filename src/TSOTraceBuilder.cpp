...
bool TSOTraceBuilder::is_available(int proc){
...
bool TSOTraceBuilder::reset(){
  ...
  /* Setup the new Event at prefix[i] */
  {
    ...
    for(int j = i-1; br_idx == 1 && 0 <= j; --j){
      if(threads[prefix[j].iid.get_pid()].cpid == br.cpid){
        br_idx = prefix[j].iid.get_index() + prefix.branch(j).size;
      }
    }

    Event evt(IID<IPid>(CPS.get_pid(br.cpid),br_idx));
...
(std::vector<std::string> &lines, std::vector<int> &iid_map,
 unsigned line, Branch branch, WakeupTreeRef<Branch> node) const{
  ...
  while(nodes.size()) {
    ...
    lines[l] += " " + iid_string(b, iid_map[b.iid.get_pid()]);
    nodes.pop_back();
  }
}
#ifndef NDEBUG
...
bool TSOTraceBuilder::post(const int tgt_th) {
  ...
  if(conf.dpor_algorithm != Configuration::EVENT_DRIVEN){
    if(0 <= last_post) {
      assert(last_post < int(prefix.len()));
...
sequence_clears_sleep(const std::vector<Branch> &seq,
  ...
  for (auto it = seq.cbegin(); state == obs_wake_res::CONTINUE
         && it != seq.cend(); ++it) {
    state = obs_sleep_wake(isleep, it->iid.get_pid(), it->sym);
  }
  /* Redundant */
...
void TSOTraceBuilder::add_eop(unsigned second, unsigned first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
  ...
  std::vector<unsigned> &vec = prefix[second].eop_before;
  if (vec.size() && vec.back() == first) return;

  vec.push_back(first);
}

void TSOTraceBuilder::add_eom(unsigned second, unsigned first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
...
void TSOTraceBuilder::add_ppm(unsigned second, unsigned first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
  ...
  std::vector<unsigned> &vec = prefix[second].ppm_before;
  if (vec.size() && vec.back() == first) return;
...
void TSOTraceBuilder::recompute_ppm_for_seq(std::map<int,Event> &wakeup_ev_seq, const int& fpid){
  for(IPid i = threads.size()-2; 0 < i; i=i-2){
    ...
    //llvm::dbgs()<<second<<prefix[i].eom_before.size()<<"\n";//////////////////////////
    for(int ei:prefix[second].eom_before){//for each eom edge
	...
	continue;
      ...
      for(auto it = vec.begin(); it != vec.end(); it++){
	if(*it == first_post){
	  //llvm::dbgs()<<"Erasing ppm "<<first_post<<second_post<<"\n";///////////////////
	  vec.erase(it);
	  break;
	}
      }
...
void TSOTraceBuilder::clear_rsc_edges(std::map<int,Event> &wakeup_ev_seq){
...
void TSOTraceBuilder::vclocks_for_seq(std::map<int,Event> &seq,
...
(const Race &race, const struct obs_sleep &isleep){
  ...
  for(int k = i+1; k <= j; ++k){
    ...
    if(isleep.count(cand.iid.get_pid())){
...
void TSOTraceBuilder::linearize_wakeup_seq(const std::map<int,Event> &wakeup_ev_seq,
...
bool TSOTraceBuilder::redundant_wakeup_seq(std::map<int,Event> wakeup_ev_seq,
  ...
  for(int i = 0; i <= ins_point; i++){//go through prefix
    for(auto s_it = post_sleep.begin(); s_it != post_sleep.end(); s_it++){
      if(s_it->second.gt(prefix[i].clock)){//erase conflicting events from post_sleep
...
(const Race &race, const struct obs_sleep &isleep){
  ...
  if(conf.dpor_algorithm == Configuration::EVENT_DRIVEN){
    recompute_ppm_for_seq(wakeup_ev_seq,fpid);
    if(threads[fpid].handler_id == threads[spid].handler_id &&
       threads[fpid].handler_id != -1){  
      /* if race is between two messages */
      ...
      std::vector<int> v1;
      linearize_wakeup_seq(wakeup_ev_seq,v1);
      clear_rsc_edges(wakeup_ev_seq);
      vclocks_for_seq(wakeup_ev_seq,v1,v);
      for(int k=0; k<i; k++) {
  ...
  if (conf.dpor_algorithm == Configuration::EVENT_DRIVEN){
    for(auto ev : wakeup_ev_seq){
      if(!ev.second.sym.empty() && ev.second.sym.front().kind == SymEv::POST)
        post_ev_clocks.insert(std::pair<IID<IPid>,
  ...
  while(1) {
      ...
      for (auto vei = v.begin(); skip == NO && vei != v.end(); ++vei) {
        ...
        if (child_it.branch() == ve) {
          ...
          if (ve.size < child_it.branch().size) {
            ...
            IPid pid = ve.iid.get_pid();
            for (auto vri = v.erase(vei); missing != 0 && vri != v.end();) {
              if (vri->iid.get_pid() == pid) {
	...
	if (conf.dpor_algorithm == Configuration::EVENT_DRIVEN &&
        ...
        else if (do_events_conflict(SPS.get_spid(ve.cpid), ve.sym,
				    SPS.get_spid(child_it.branch().cpid),
				    child_sym, false)) {
...
wakeup_sequence(const Race &race, std::map<int,Event> &wakeup_ev_seq) const{
  int i = race.first_event;
  int j = race.second_event;
  ...
  if (race.kind == Race::LOCK_FAIL) {
    ...
    second_br = Branch(second.iid,
                       threads[second.iid.get_pid()].cpid);
  ...
  if (race.kind == Race::OBSERVED) {
    ...
    v.back().cpid = threads[prefix[k].iid.get_pid()].cpid;
  }
