...
bool POPTraceBuilder::schedule(int *proc, int *aux, int *alt, bool *dryrun){
...
void POPTraceBuilder::refuse_schedule(){
...
void POPTraceBuilder::mark_unavailable(int proc, int aux){
  threads[ipid(proc,aux)].available = false;
}

bool POPTraceBuilder::is_replaying() const {
  return prefix_idx < replay_point;
}

void POPTraceBuilder::cancel_replay(){
  if(!replay) return;
  replay = false;
...
void POPTraceBuilder::metadata(const llvm::MDNode *md){
  if(!dryrun){
    curev().md = md;
  }
  last_md = md;
...
bool POPTraceBuilder::sleepset_is_empty() const{
...
bool POPTraceBuilder::check_for_cycles(){
...
Trace *POPTraceBuilder::get_trace() const{
...
bool POPTraceBuilder::reset(){
  ...
  if(conf.debug_print_on_reset){
    llvm::dbgs() << " === POPTraceBuilder reset ===\n";
    debug_print();
    llvm::dbgs() << " =============================\n";
  }
...
IID<CPid> POPTraceBuilder::get_iid() const{
  return get_iid(prefix_idx);
}
IID<CPid> POPTraceBuilder::get_iid(unsigned i) const{
...
std::string POPTraceBuilder::iid_string(std::size_t pos) const{
  return iid_string(prefix[pos], prefix[pos].iid.get_index());
}

std::string POPTraceBuilder::iid_string(const Event &event, int index) const{
  std::stringstream ss;
...
#ifndef NDEBUG
void POPTraceBuilder::check_symev_vclock_equiv() const {
...
print_conflict_map(const conflict_map_t &conflict_map) const {
  for(const auto &cfl_node : conflict_map) {
...
void POPTraceBuilder::
print_conflict_maps(const std::vector<conflict_map_t> &conflict_maps) const {
  for(const auto &conflict_map : conflict_maps)
...
bool POPTraceBuilder::store(const SymData &sd){
...
bool POPTraceBuilder::atomic_store(const SymData &sd){
  if (conf.dpor_algorithm == Configuration::OBSERVERS) {
...
void POPTraceBuilder::do_atomic_store(const SymData &sd){
...
bool POPTraceBuilder::atomic_rmw(const SymData &sd, RmwAction action) {
  curev().may_conflict = true;
...
bool POPTraceBuilder::xchg_await(const SymData &sd, AwaitCond cond) {
  curev().may_conflict = true;
...
bool POPTraceBuilder::xchg_await_fail(const SymData &sd, AwaitCond cond) {
...
bool POPTraceBuilder::load(const SymAddrSize &ml){
  if (!record_symbolic(SymEv::Load(ml))) return false;
  do_load(ml);
  return true;
}

void POPTraceBuilder::do_load(const SymAddrSize &ml){
  curev().may_conflict = true;
  IPid ipid = curev().iid.get_pid();
...
bool POPTraceBuilder::compare_exchange
...
bool POPTraceBuilder::load_await(const SymAddrSize &ml, AwaitCond cond) {
  curev().may_conflict = true;
  if (conf.memory_model != Configuration::SC) {
...
bool POPTraceBuilder::load_await_fail(const SymAddrSize &ml, AwaitCond cond) {
  if (conf.memory_model != Configuration::SC) {
    invalid_input_error("Load-Await is only implemented for SC right now");
    return false;
  }
...
bool POPTraceBuilder::do_await(unsigned j, const IID<IPid> &iid, const SymEv &e,
...
bool POPTraceBuilder::full_memory_conflict(){
  curev().may_conflict = true;
...
void POPTraceBuilder::observe_memory(SymAddr ml, ByteInfo &m,
                                     VecSet<int> &seen_accesses,
                                     VecSet<std::pair<int,int>> &seen_pairs,
                                     bool is_update){
  IPid ipid = curev().iid.get_pid();
...
bool POPTraceBuilder::join(int tgt_proc){
  curev().may_conflict = true;
...
bool POPTraceBuilder::mutex_lock(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool POPTraceBuilder::mutex_lock_fail(const SymAddrSize &ml){
  assert(!dryrun);
  if(!conf.mutex_require_init && !mutexes.count(ml.addr)){
    // Assume static initialization
    mutexes[ml.addr] = Mutex();
...
bool POPTraceBuilder::mutex_trylock(const SymAddrSize &ml){
  curev().may_conflict = true;
  if (!record_symbolic(SymEv::MLock(ml))) return false;
  if (!fence()) return false;
  if(!conf.mutex_require_init && !mutexes.count(ml.addr)){
...
bool POPTraceBuilder::mutex_unlock(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool POPTraceBuilder::mutex_init(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool POPTraceBuilder::mutex_destroy(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool POPTraceBuilder::cond_init(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool POPTraceBuilder::cond_signal(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool POPTraceBuilder::cond_broadcast(const SymAddrSize &ml){
  curev().may_conflict = true;
...
bool POPTraceBuilder::cond_wait(const SymAddrSize &cond_ml, const SymAddrSize &mutex_ml){
  {
...
int POPTraceBuilder::cond_destroy(const SymAddrSize &ml){
...
bool POPTraceBuilder::register_alternatives(int alt_count){
  curev().may_conflict = true;
...
add_conflict_map(std::vector<conflict_map_t> &conflict_map,
  ...
  for(auto cfl_node : local_conflict_map){
    ...
    cfl_map_stack.emplace_back(&(iconflict_map.back().inherited));
    while(!stack.empty()){
      // pop stack
      auto curr = std::move(stack.back());
      auto cfl_map_curr = std::move(cfl_map_stack.back());
      stack.pop_back();
      cfl_map_stack.pop_back();
      // visit node
	...
	continue;
      ...
      cfl_map_stack.emplace_back(cfl_map_curr);
      // push right node
      if(curr_node->inherited != nullptr){
        stack.emplace_back(curr_node->inherited, 0);
        cfl_map_stack.emplace_back(&(cfl_map_curr->back().inherited));
      }
    }
  }
...
POPTraceBuilder::update_conflict_res
POPTraceBuilder::update_conflict_map(std::vector<conflict_map_t> &conflict_maps,
				     IPid p, const sym_ty &sym,
				     const VClock<IPid> &clock) const {
  bool all_clear = true;
  std::list<std::pair<conflict_map_t *, int>> queue;
  std::vector<std::list<std::pair<conflict_map_t *, unsigned>>> stacks;
  for(auto &conflict_map : conflict_maps){
    if(conflict_map.empty()) continue;
    const auto &base_addr = conflict_map.front().addr;
    queue.emplace_back(&conflict_map, -1);
    while(!queue.empty()){
      auto curr_conflict_map = queue.back().first;
      auto stack_no = queue.back().second;
      queue.pop_back();
      assert(!curr_conflict_map->empty());

      bool addr_overlaps = false;
      bool load_overlaps = false;
	...
	if(symev.has_addr() &&
	  ...
	  else if(symev_does_load(symev)) load_overlaps = true;
	}
      }
      
      if(store_overlaps){
	curr_conflict_map->clear();
	break;
      }
      
      update_conflict_res res = update_conflict_res::CONTINUE;
      for(unsigned index = 0; index < curr_conflict_map->size(); index++){
	auto &cfl_node = curr_conflict_map->at(index);
	auto &same_var_load_clks = curr_conflict_map->front().same_var_load_clks;
	// Update this node
	if(!cfl_node.cfl_detector.H.empty() ||
	  ...
	  if(res == update_conflict_res::BLOCK && addr_overlaps &&
	    ...
	    if(stack_no != -1){
	      while(!stacks[stack_no].empty()){
		auto same_var_load_clks =
		  stacks[stack_no].front().first->front().same_var_load_clks;
		auto backtrack_conflict_map = stacks[stack_no].back().first;
		unsigned index = stacks[stack_no].back().second;
		stacks[stack_no].pop_back();
		if(backtrack_conflict_map->at(index).cfl_detector.
		   schedules_clock.lt(clock)){
		  if(is_base_addr){
		    if(stacks[stack_no].empty()){
		      bool happens_after_another_load = false;
		      for(const auto &clk : same_var_load_clks)
			if(clk.lt(clock)){
			  happens_after_another_load = true;
			  break;
			}
		      if(!happens_after_another_load)
			return update_conflict_res::BLOCK;
		    }
		  }
		  else
		    backtrack_conflict_map->front().slp_ev_clks.push_back(clock);
		}
		else break;
	      }
	    }
	...
	if(load_overlaps && index == 0)
	  cfl_node.same_var_load_clks.push_back(clock);
	if(res != update_conflict_res::CONTINUE) break;
	if(!cfl_node.inherited.empty()){
	  int inherited_stack_no = -1;
	  if(load_overlaps &&
	     cfl_node.addr == cfl_node.inherited.front().addr){
	    inherited_stack_no = stack_no;
	    if(inherited_stack_no == -1){
	      inherited_stack_no = stacks.size();
	      stacks.push_back(std::list<std::pair<conflict_map_t *, unsigned>>());
	    }
	    stacks[inherited_stack_no].emplace_back(curr_conflict_map, index);
	  }
	  queue.emplace_back(&(cfl_node.inherited), inherited_stack_no);
	}
	...
	if(load_overlaps){
	  ...
	  if(!cfl_node.inherited.empty()){
		...
		if(res == update_conflict_res::CLEAR) break;
...
bool POPTraceBuilder::
blocked_wakeup_sequence(std::vector<Event> &seq,
...
bool POPTraceBuilder::awaitcond_satisfied_before
...
bool POPTraceBuilder::awaitcond_satisfied_by
...
void POPTraceBuilder::recompute_cmpxhg_success
...
void POPTraceBuilder::see_event_pairs
...
void POPTraceBuilder::add_noblock_race(int event){
  assert(0 <= event);
...
void POPTraceBuilder::add_lock_suc_race(int lock, int unlock){
  assert(0 <= lock);
  assert(lock < unlock);
...
void POPTraceBuilder::add_lock_fail_race(int event){
  assert(0 <= event);
  assert(event < prefix_idx);
...
void POPTraceBuilder::add_observed_race(int first, int second){
  assert(0 <= first);
  assert(first < second);
...
void POPTraceBuilder::add_happens_after(unsigned second, unsigned first){
  assert(first != ~0u);
  assert(second != ~0u);
  assert(first != second);
  assert(first < second);
...
void POPTraceBuilder::add_happens_after_thread(unsigned second, IPid thread){
  assert((int_fast64_t)second == prefix_idx);
...
void POPTraceBuilder::compute_vclocks(){
  /* Be idempotent */
...
bool POPTraceBuilder::record_symbolic(SymEv event){
  if (!replay) {
...
bool POPTraceBuilder::do_events_conflict(int i, int j) const{
  return do_events_conflict(prefix[i], prefix[j]);
}

bool POPTraceBuilder::do_events_conflict
...
bool POPTraceBuilder::do_symevs_conflict
...
bool POPTraceBuilder::is_observed_conflict
...
bool POPTraceBuilder::do_race_detect() {
  ...
  for (unsigned j = 0; j < prefix.size(); ++j){
	...
	prefix.back().local_conflict_map = std::move(local_conflict_map);
...
std::vector<int> POPTraceBuilder::iid_map_at(int event) const{
...
void POPTraceBuilder::iid_map_step(std::vector<int> &iid_map, const Event &event) const{
  if (iid_map.size() <= unsigned(event.iid.get_pid())) iid_map.resize(event.iid.get_pid()+1, 1);
  iid_map[event.iid.get_pid()] += event.size;
}

void POPTraceBuilder::iid_map_step_rev(std::vector<int> &iid_map, const Event &event) const{
  iid_map[event.iid.get_pid()] -= event.size;
}

VClock<int> POPTraceBuilder::reconstruct_blocked_clock(IID<IPid> event) const {
  VClock<IPid> ret;
...
POPTraceBuilder::Event POPTraceBuilder::
...
inline std::pair<bool,unsigned> POPTraceBuilder::
...
inline unsigned POPTraceBuilder::find_process_event(IPid pid, int index) const{
  assert(pid >= 0 && pid < int(threads.size()));
...
bool POPTraceBuilder::has_pending_store(IPid pid, SymAddr ml) const {
...
void POPTraceBuilder::wakeup(Access::Type type, SymAddr ml){
}

bool POPTraceBuilder::has_cycle(IID<IPid> *loc) const{
...
long double POPTraceBuilder::estimate_trace_count() const{
  return estimate_trace_count(0);
}

long double POPTraceBuilder::estimate_trace_count(int idx) const{
